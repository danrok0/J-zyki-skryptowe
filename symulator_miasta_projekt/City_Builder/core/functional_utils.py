...
def retry_on_failure(max_attempts: int = 3, delay: float = 1.0):
    ...
    def decorator(func: Callable) -> Callable:
        # @wraps(func) zachowuje metadane oryginalnej funkcji (nazwa, docstring, etc.)
        # Bez tego wrapper.__name__ byłby "wrapper" zamiast nazwy oryginalnej funkcji
        # Ważne dla debugowania i introspekci kodu
        @wraps(func)
        def wrapper(*args, **kwargs):
...
def memoize(func: Callable) -> Callable:
    ...
    # @wraps zachowuje oryginalną sygnaturę funkcji i metadane
    # Dzięki temu zamemoizowana funkcja wygląda jak oryginalna dla zewnętrznego kodu
    @wraps(func)
    def wrapper(*args, **kwargs):
        # Tworzenie klucza cache
        # str() konwertuje argumenty na string dla użycia jako klucz słownika
        # sorted() sortuje kwargs aby zapewnić konsystentny klucz niezależnie od kolejności
...
def calculate_resource_trends(resource_history: List[Dict[str, float]]) -> Dict[str, Any]:
    ...
    # set() tworzy zbiór unikalnych nazw zasobów ze wszystkich wpisów historii
    all_resources = set()
    ...
    for resource in all_resources:
        # Wyciągnij wartości dla danego zasobu używając map() z lambdą
        # map() aplikuje lambda do każdego elementu resource_history
        # lambda entry: entry.get(resource, 0) pobiera wartość zasobu lub 0 jako domyślną
        values = list(map(
        ...
        # Oblicz zmiany między kolejnymi wartościami używając map() z lambdą
        # range(1, len(values)) tworzy indeksy od 1 do końca
        # lambda i: values[i] - values[i-1] oblicza różnicę między kolejnymi wartościami
        # To daje nam listę zmian: [v1-v0, v2-v1, v3-v2, ...]
        changes = list(map(
            lambda i: values[i] - values[i-1],
            range(1, len(values))
        ))
        
        # Oblicz statystyki zmian używając reduce()
        # reduce(lambda a, b: a + b, changes, 0) sumuje wszystkie zmiany
        # lambda a, b: a + b to funkcja dodawania dwóch argumentów
        # 0 to wartość początkowa (initial value) dla reduce
        total_change = reduce(lambda a, b: a + b, changes, 0)
        avg_change = total_change / len(changes)
        
        # Określ trend używając filter() z lambdami
        # filter(lambda c: c > 0, changes) filtruje tylko pozytywne zmiany
        # lambda c: c > 0 to predykat sprawdzający czy zmiana jest dodatnia
        positive_changes = list(filter(lambda c: c > 0, changes))
        # filter(lambda c: c < 0, changes) filtruje tylko negatywne zmiany  
        # lambda c: c < 0 to predykat sprawdzający czy zmiana jest ujemna
        ...
        trends[resource] = {
            ...
            # Oblicz zmienność (volatility) używając filter() z lambdą
            # filter(lambda c: abs(c) < 0.1, changes) filtruje małe zmiany (stabilne)
            # lambda c: abs(c) < 0.1 sprawdza czy absolutna wartość zmiany < 0.1
            # volatility = liczba wszystkich zmian - liczba stabilnych zmian
            'volatility': len(changes) - len(list(filter(lambda c: abs(c) < 0.1, changes))),
            'current_value': values[-1],
            # reduce(lambda a, b: max(a, b), values) znajdzie maksymalną wartość
            # lambda a, b: max(a, b) porównuje dwa elementy i zwraca większy
            'peak_value': reduce(lambda a, b: max(a, b), values),
            # reduce(lambda a, b: min(a, b), values) znajdzie minimalną wartość
            # lambda a, b: min(a, b) porównuje dwa elementy i zwraca mniejszy
...
def optimize_city_layout(city_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # defaultdict(list) tworzy słownik gdzie każdy nowy klucz automatycznie otrzymuje pustą listę
    # Dzięki temu nie musimy sprawdzać czy klucz istnieje przed dodaniem elementu
    ...
    for building_type, type_buildings in building_groups.items():
        # Analizuj rozmieszczenie budynków tego typu używając map() z lambdą
        # map() przekształca każdy budynek na tuple współrzędnych (x, y)
        # lambda b: (getattr(b, 'x', 0), getattr(b, 'y', 0)) pobiera pozycję budynku
        # getattr(obj, 'attr', default) bezpiecznie pobiera atrybut lub zwraca wartość domyślną
        positions = list(map(
        ...
        # Oblicz średnią pozycję (centrum masy) używając reduce() i map()
        # map(lambda p: p[0], positions) wyciąga wszystkie współrzędne x z pozycji
        # lambda p: p[0] pobiera pierwszy element tuple (współrzędną x)
        # reduce(lambda a, b: a + b, ...) sumuje wszystkie wartości x
        avg_x = reduce(lambda a, b: a + b, map(lambda p: p[0], positions)) / len(positions)
        # map(lambda p: p[1], positions) wyciąga wszystkie współrzędne y z pozycji  
        # lambda p: p[1] pobiera drugi element tuple (współrzędną y)
        avg_y = reduce(lambda a, b: a + b, map(lambda p: p[1], positions)) / len(positions)
        
        # Oblicz rozproszenie (odległości od centrum) używając map() z lambdą
        # map() oblicza odległość euklidesową każdej pozycji od centrum masy
        # lambda p: ((p[0] - avg_x) ** 2 + (p[1] - avg_y) ** 2) ** 0.5 to wzór na odległość
        # (x1-x2)² + (y1-y2)² = kwadrat odległości, ** 0.5 = pierwiastek kwadratowy
        distances = list(map(
            lambda p: ((p[0] - avg_x) ** 2 + (p[1] - avg_y) ** 2) ** 0.5,
            positions
        ))
        
        # Oblicz średnią odległość używając reduce()
        # reduce(lambda a, b: a + b, distances) sumuje wszystkie odległości
        # / len(distances) daje średnią odległość od centrum
...
def validate_game_data(data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    """
    # defaultdict(list) tworzy słownik gdzie każdy nowy klucz automatycznie otrzymuje pustą listę
    # Gdy próbujemy dodać element do nieistniejącego klucza: errors['nowy_klucz'].append(...)
    # zamiast KeyError dostajemy automatycznie utworzoną pustą listę
    # To eliminuje potrzebę sprawdzania if key in dict przed dodaniem elementu
...
def compose(*functions):
    ...
    """
    # reduce() komponuje funkcje od prawej do lewej: f(g(h(x)))
    # lambda f, g: lambda x: f(g(x)) tworzy funkcję kompozytową z dwóch funkcji
    # reduce aplikuje tę kompozycję do wszystkich funkcji w sekwencji
    # lambda x: x to funkcja tożsamościowa (identity function) jako wartość początkowa
    # Przykład: compose(f, g, h) = lambda x: f(g(h(x)))
...
def curry(func: Callable) -> Callable:
    ...
    def curried(*args, **kwargs):
        if len(args) + len(kwargs) >= func.__code__.co_argcount:
            return func(*args, **kwargs)
        # lambda *more_args, **more_kwargs: ... tworzy nową funkcję czekającą na więcej argumentów
        # Rekurencyjne wywołanie curried() z połączonymi argumentami
        # *(args + more_args) rozpakuje tuple argumentów pozycyjnych
        # **{**kwargs, **more_kwargs} scala słowniki argumentów kluczowych
...
def pipe(value: Any, *functions) -> Any:
    ...
    """
    # reduce() aplikuje funkcje od lewej do prawej: f3(f2(f1(value)))
    # lambda v, f: f(v) aplikuje funkcję f do wartości v
    # value to wartość początkowa dla reduce
    # Przykład: pipe(10, add1, multiply2, subtract3) = subtract3(multiply2(add1(10)))
